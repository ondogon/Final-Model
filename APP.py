...
st.title("🌱 식물 스트레스 예측 — 최종 모델(실험점 보정 + LSTM 블렌드)")

with st.sidebar:
    uv_pick = st.radio("자외선 조사 시간 선택", [15, 30], index=0, horizontal=True)
    show_markers = st.checkbox("마커 표시", value=True)
    st.caption("• 시간축은 0–120분\n• 호버(마우스 오버)로 값 확인")
...
fig.update_layout(
    title=f"UV {uv_pick}분 — 예측 곡선(실험점 보정 + 가설형태 블렌드)",
...
st.caption("※ 곡선은 실험 타깃 시점을 정확히 통과하며, 구간 중앙에서만 LSTM 원시 곡선을 소량 블렌딩합니다.")
